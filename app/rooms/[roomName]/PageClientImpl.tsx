...
function VideoConferenceComponent(props: {
  ...
  // Generate or retrieve meeting ID for proper database scoping
  React.useEffect(() => {
    if (typeof window !== 'undefined') {
      let meetingId = sessionStorage.getItem('eburon_meeting_id');
      
      // Generate a new meeting ID if none exists or if it's malformed
      if (!meetingId || meetingId.includes(':')) {
        meetingId = `MEETING_${Math.random().toString(36).substring(2, 9).toUpperCase()}`;
        sessionStorage.setItem('eburon_meeting_id', meetingId);
        console.log('Generated new meeting ID:', meetingId);
      } else {
...
// Actually, I can use multi_replace for this.
  ...
  const handleTranscriptSegment = React.useCallback(async (segment: any) => {
    ...
    // Get the generated meeting ID from sessionStorage
    const meetingId = typeof window !== 'undefined' ? sessionStorage.getItem('eburon_meeting_id') : null;
    
    if (!meetingId) {
      console.log('?? No meeting ID found, exiting');
      return;
    }
    
    console.log('?? meeting ID:', meetingId);
    console.log('?? user:', user);
    console.log('?? shouldSuppressTTS:', segment.shouldSuppressTTS);
    
    // If this is the current user speaking, suppress TTS
    if (segment.shouldSuppressTTS || segment.isCurrentUser) {
      console.log('?? Suppressing TTS for current user');
      // Here you would add TTS suppression logic
      // For example: stop any currently playing TTS, mark this segment to not be read aloud
      return;
    }
    
    try {
        const speakerId = segment.speakerId || user?.id || crypto.randomUUID();
        const insertData = {
            meeting_id: meetingId,
            speaker_id: speakerId,
            user_id: segment.userId || user?.id || null,
            transcribe_text_segment: segment.text,
            full_transcription: segment.text,
            is_current_user: segment.isCurrentUser || false,
            suppress_tts: segment.shouldSuppressTTS || false,
        };
        ...
        const result = await supabase.from('transcriptions').insert({
            meeting_id: meetingId,
            speaker_id: speakerId,
            user_id: segment.userId || null, // Set to null to avoid FK constraint - user may not exist in users table
            transcribe_text_segment: segment.text,
            full_transcription: segment.text,
            is_current_user: segment.isCurrentUser || false,
            suppress_tts: segment.shouldSuppressTTS || false,
